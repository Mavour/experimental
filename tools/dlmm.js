...
export async function getPositionPnl({ pool_address, position_address }) {
  ...
  try {
    ...
    const unclaimedFeeTokenYUsd = parseFloat(p.unrealizedPnl?.unclaimedFeeTokenY?.usd || 0);
    const currentValueUsd = parseFloat(p.unrealizedPnl?.balances || 0);
    return {
      pnl_usd:           Math.round((p.pnlUsd ?? 0) * 100) / 100,
      pnl_sol:           Math.round((p.pnlSol ?? p.pnlUsd ?? 0) * 10000) / 10000,
      pnl_pct:           Math.round((p.pnlPctChange ?? 0) * 100) / 100,
      pnl_sol_pct:       Math.round((p.pnlSolPctChange ?? p.pnlPctChange ?? 0) * 100) / 100,
      current_value_usd: Math.round(currentValueUsd * 100) / 100,
      unclaimed_fee_usd: Math.round(unclaimedUsd * 100) / 100,
      unclaimed_fee_tokeny_usd: Math.round(unclaimedFeeTokenYUsd * 100) / 100,
    ...
    for (const pool of pools) {
      for (const positionAddress of (pool.listPositions || [])) {
        ...
        positions.push({
          ...
          in_range:           !isOOR,
          unclaimed_fees_usd: Math.round(parseFloat(pool.unclaimedFees || 0) * 10000) / 10000,
          unclaimed_fees_sol: Math.round(parseFloat(pool.unclaimedFeesSol || pool.unclaimedFees || 0) * 10000) / 10000,
          total_value_usd:    Math.round(parseFloat(pool.balances || 0) * 10000) / 10000,
          total_value_sol:    Math.round(parseFloat(pool.balancesSol || pool.balances || 0) * 10000) / 10000,
          collected_fees_usd: Math.round(parseFloat(pool.unclaimedFees || 0) * 10000) / 10000,
          pnl_usd:            Math.round(parseFloat(pool.pnl || 0) * 10000) / 10000,
          pnl_sol:            Math.round(parseFloat(pool.pnlSol || pool.pnl || 0) * 10000) / 10000,
          pnl_pct:            Math.round(parseFloat(pool.pnlPctChange || 0) * 100) / 100,
          pnl_sol_pct:        Math.round(parseFloat(pool.pnlSolPctChange || pool.pnlPctChange || 0) * 100) / 100,
  ...
  try {
    ...
    const positions = raw.map((r) => {
      ...
      return {
        ...
        unclaimed_fee_tokeny_usd: Math.round((p ? parseFloat(p.unrealizedPnl?.unclaimedFeeTokenY?.usd || 0) : 0) * 100) / 100,
